...
def tag_special_ingredients(dish):
    ...
    ingrendients = dish[1]
    specials = set(ingrendients).intersection(SPECIAL_INGREDIENTS)
...
def compile_ingredients(dishes):
    ...
    for sets in dishes:
        for item in sets:
            compiled_dishes.append(item)
    return set(compiled_dishes)
...
def separate_appetizers(dishes, appetizers):
    ...
    removed_appetizers = set(dishes).difference(appetizers)
    new_dishes = []
    for item in removed_appetizers:
        new_dishes.append(item)
    return new_dishes
...
def singleton_ingredients(dishes, intersection):
    ...
    for sets in dishes:
        for item in sets:
            list_dishes.append(item) 
    unique_ingredients = set(list_dishes).difference(intersection)
